...
const NotificationItem = ({ item, onDelete, onModal }) => {
  ...
  return (
    <div className={`not-item fs-5 bg-white fw-bold btn-default px-3 d-flex align-items-center rounded-0 border-0 border-bottom border-black cursor-default position-relative ${item.read === 1 && 'reader'}`}
  ...
  return (
    <div id="documentsPage" className="flex-fill h-100">
      ...
      <div id="notification" className="px-2 px-sm-4 my-4" aria-live="polite">
        ...
        <div className="btn-group rounded-0 rounded-top" role="group">
          <button className={`btn teste fs-6 px-2 rounded-0 rounded-top ${filter === 'all' && 'bg-primary text-white active'}`} onClick={() => setFilter("all")}>Todas as notificações</button>
          <button className={`btn teste fs-6 px-2 rounded-0 rounded-top ${filter === 'read' && 'bg-primary text-white active'}`} onClick={() => setFilter("read")}>Notificações lidas</button>
          <button className={`btn teste fs-6 px-2 rounded-0 rounded-top ${filter === 'unread' && 'bg-primary text-white active'}`} onClick={() => setFilter("unread")}>Notificações não lidas</button>
        </div>

        {filteredNotifications.length > 0 ? (
          <div className="border border-top-0 border-bottom-0 border-black">
