...
function Modeler(props) {
    ...
    async function updateDiagram() {
        ...
        try {
            ...
            await api.post('notification', {user_id: filtered_user_ids, diagram_id: id, type: 2, message: `"${collaborator_name}" editou o ${owner ? 'diagrama compartilhado com você' : 'seu diagrama'}: "${name}"`})
    ...
    async function rename(e = null) {
        ...
        try {
            ...
            if(!owner && nameAntes !== name){
                const collaborator_name = JSON.parse(localStorage.getItem('user')).name;
                await api.post('notification', {user_id: user_id, diagram_id: id, type: 2, message: `"${collaborator_name}" alterou o nome do seu diagrama: "${nameAntes}" para "${name}"`})
