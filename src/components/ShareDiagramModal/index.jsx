...
function ShareDiagramModal(props) {
    ...
    async function inviteLink() {        
        ...
        try {            
            ...
            await api.post('notification', {user_id: user_id, diagram_id: props.diagram_id, type: 1, message: `"${collaborator_name}" compartilhou o diagrama: "${name}. Cheque seu e-mail!"`})
    ...
    async function updatePermission(user_id, updation) {
        try{
            ...
            if(updation === "StopShare"){
                ...
                await api.post('notification', {user_id: user_id, diagram_id: props.diagram_id, type: 1, message: `"${collaborator_name}" parou de compartilhar o diagrama: "${name}"`})     
                await socket.emit('send_notification', user_id);    
                getAllCollaborations();                
            } else {
                await api.put(`/collaboration/${props.diagram_id}/${user_id}`, {updation});
                await api.post('notification', {user_id: user_id, diagram_id: props.diagram_id, type: 1, message: `"${collaborator_name}" deu permissão de ${updation === '1' ? 'leitor' : 'editor'} no: "${name}"`})   
                await socket.emit('send_notification', user_id);
            }         
        } catch(error) {
            if(error === "TypeError: Cannot read properties of undefined (reading 'status')"){
